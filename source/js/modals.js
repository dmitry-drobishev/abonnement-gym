...
export class Modals {
  ...
  _init() {
    ...
    // }
    document.querySelector('[data-open-modal]').dataset.openModal;
    this.open();
  }
  ...
  _documentKeydownHandler(evt) {
    ...
    if (isEscKey) {
      evt.preventDefault();
      this.close(document.querySelector('.popup.popup--visible').dataset.modal);
    }
  }
  ...
  open(modalName = this._modalName) {
    const modal = document.querySelector(`[data-modal="popup"]`);

    if (!modal || modal.classList.contains('popup--visible')) {
      return;
    }

    document.removeEventListener('click', this._documentClickHandler);

    this._openedModalElement = document.querySelector('.popup.popup--visible');
    ...
    if (this._lockFocus) {
      this._focusLock.lock('.popup.popup--visible', this._startFocus);
    }
